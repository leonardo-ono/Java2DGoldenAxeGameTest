    ...
    private void updateEnemiesStrategy() {
        ...
        if (System.currentTimeMillis() > strategyChangeTime || enemyKilled) {
            ...
            for (Entity entity : stage.getEntities()) {
                ...
                else if (entity instanceof Beast1) {
                    Beast1 beast = (Beast1) entity;
                    if (!beast.isDead() && (beast.getMountedBy() instanceof Enemy1 || beast.getMountedBy() instanceof Enemy3)) {
                        beast.setAIMode(aiModes[enemyIndex++]);
                    }
                }
            }
